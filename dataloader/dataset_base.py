...
class BaseDataset(Dataset):  # pylint: disable=locally-disabled, invalid-name
    ...
    def __init__(self,
        ...
        # route_dirs = glob.glob(os.path.join(self.data_path, 'Town*'))
        
        print(f"Recursively scanning for routes in {self.data_path}...")
        route_dirs = []
        for root, dirs, files in os.walk(self.data_path):
            for dir_name in dirs:
                if dir_name.startswith('Town'):
                    route_dirs.append(os.path.join(root, dir_name))
